...
const MerchantReports = () => {
  ...
  const [isDownloadAdminOpen, setIsDownloadAdminOpen] = useState(false);
  const [isDownloadUserOpen, setIsDownloadUserOpen] = useState(false);
  ...
  return (
    <section className="content-section">
      ...
      <div style={accordionContainerStyle}>
        ...
        {/* How to Download as System Administrator */}
        <div>
          <div style={accordionHeaderStyle(isDownloadAdminOpen)} onClick={() => setIsDownloadAdminOpen(!isDownloadAdminOpen)}>
            <span style={{ fontSize: '15px', fontWeight: '500' }}>How to Download Billing Zipped Reports as System Administrator role type:</span>
            <span style={chevronStyle(isDownloadAdminOpen)}>&#8964;</span>
          </div>
          {isDownloadAdminOpen && (
        ...
        <div>
          <div style={{ ...accordionHeaderStyle(isDownloadUserOpen), borderBottom: isDownloadUserOpen ? '1px solid #ddd' : 'none' }} onClick={() => setIsDownloadUserOpen(!isDownloadUserOpen)}>
            <span style={{ fontSize: '15px', fontWeight: '500' }}>How to Download Merchant Billing Zipped Reports as User/User Administrator role type:</span>
            <span style={chevronStyle(isDownloadUserOpen)}>&#8964;</span>
          </div>
          {isDownloadUserOpen && (
            <div style={{ ...accordionBodyStyle, borderBottom: 'none' }}>
              ...
              <div style={{ textAlign: 'center', marginBottom: '10px', marginTop: '10px' }}>
                <img src={downloadDialogImage} alt="Download Dialog Box" style={{ width: '50%' }} />
